...
public final class DatabaseUtils {
    ...
    public static <T> List<T> newList(Cursor cursor, Class<T> componentType) {
        try {
            DebugUtils._checkError(componentType.isPrimitive(), "Unsupported primitive type - " + componentType.toString());
    ...
    public static JsonWriter writeCursorRow(JsonWriter writer, Cursor cursor, int[] columnIndexes, String[] names) throws IOException {
        DebugUtils._checkError(columnIndexes.length != names.length, "columnIndexes.length != names.length");
        writer.beginObject();
        for (int i = 0; i < columnIndexes.length; ++i) {
            ...
            case Cursor.FIELD_TYPE_BLOB:
                DebugUtils._checkError(true, "Unsupported column type - BLOB");
                break;
            }
        }
    ...
    private static <T> T newInstanceImpl(Cursor cursor, Class<? extends T> clazz, List<Field> fields) throws ReflectiveOperationException {
        DebugUtils._checkError(clazz == Object.class || clazz == Void.class || clazz == String.class || (clazz.getModifiers() & (Modifier.ABSTRACT | Modifier.INTERFACE)) != 0, "Unsupported class type - " + clazz.toString());
