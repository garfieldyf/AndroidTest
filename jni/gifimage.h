...
JNIEXPORT_METHOD(jboolean) nativeDraw(JNIEnv* env, jclass /*clazz*/, jobject bitmapCanvas, jlong nativeImage, jint frameIndex)
{
    ...
    if (result)
    {
        ...
        assert_log(info.width >= image->getWidth(), "The bitmap canvas width must be >= GIF image width");
        assert_log(info.height >= image->getHeight(), "The bitmap canvas height must be >= GIF image height");
